...
async function setupDatabase() {
  try {
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS team_invitations (
        ...
        invitation_type TEXT DEFAULT 'teammate',
        team_side TEXT,
    ...
    try {
      ...
      // This will set a default value for any existing records
      await db.execute(`
        UPDATE team_invitations 
        SET inviter_username = 'Unknown User' 
      ...
      console.log(
        "Migration note: inviter_username column handling:",
        err instanceof Error ? err.message : "Unknown error"
      );
    }

    // Add invitation_type column to existing tables (migration)
    try {
      await db.execute(`
        ALTER TABLE team_invitations 
      ...
      // If teamId is 'new-opposing-team', it's an opponent invitation
      await db.execute(`
        UPDATE team_invitations 
      ...
      console.log(
        "Migration note: invitation_type column handling:",
        err instanceof Error ? err.message : "Unknown error"
      );
    }
  ...
  CREATE TABLE IF NOT EXISTS questions (
    ...
    await db.execute(`
      CREATE INDEX IF NOT EXISTS idx_team_battles_captain_id ON team_battles(team_a_captain_id, team_b_captain_id);
    `);
