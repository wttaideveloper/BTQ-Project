...
import { Pause, Play, Gamepad2, Target, User, Users, Home, RotateCcw, AlertTriangle, HelpCircle, X, Trophy, LogOut } from 'lucide-react';
...
import { initSounds, isVoiceEnabled, stopSpeaking, playSound } from '@/lib/sounds';
...
const Game: React.FC = () => {
  ...
  // Generate a unique game session ID for local multiplayer games
  const gameId = gameMode === 'multi' ? 
    ...
    try {
      ...
      if (saved) {
        ...
        return {
          ...
          playerStats: state.playerStats || Array.from({ length: playerCount }, () => ({
            score: 0, correctAnswers: 0, incorrectAnswers: 0, totalTimeSpent: 0, averageTime: 0
          })),
          gameTimeRemaining: state.gameTimeRemaining || (gameType === 'time' ? 15 * 60 : 0),
        };
      }
    } catch (error) {
  ...
  const [incorrectAnswers, setIncorrectAnswers] = useState(savedState?.incorrectAnswers ?? 0);
  const [totalTimeSpent, setTotalTimeSpent] = useState(savedState?.totalTimeSpent ?? 0);
  const [isQuestionAnswered, setIsQuestionAnswered] = useState(false);
  const [avatarMessage, setAvatarMessage] = useState("Let's see how well you know the Bible!");
  ...
  const [currentReward, setCurrentReward] = useState<'book' | 'cap' | 'tshirt' | 'certificate' | null>(null);
  const [leaderboardData, setLeaderboardData] = useState<any[]>([]);
  const [gameEnded, setGameEnded] = useState(false);
  const [currentPlayerIndex, setCurrentPlayerIndex] = useState(savedState?.currentPlayerIndex ?? 0); // Track which player's turn it is
  ...
  const [gameTimeRemaining, setGameTimeRemaining] = useState(savedState?.gameTimeRemaining ?? (gameType === 'time' ? 15 * 60 : 0)); // 15 minutes in seconds
  const [originalGameTime, setOriginalGameTime] = useState(gameType === 'time' ? 15 * 60 : 0); // Store original time for pause calculations
  
  const timeLimit = 20; // 20 seconds per question
  
  // Fetch questions from API for game
  const { data: questions, isLoading, error } = useQuery({
    queryKey: ['/api/game/questions', category, difficulty],
    queryFn: () => getGameQuestions(category, difficulty, gameType === 'question' ? 10 : 20),
  });
  ...
  const stats = {
    correctAnswers,
    incorrectAnswers,
    averageTime: (correctAnswers + incorrectAnswers) > 0 
  ...
  useEffect(() => {
    ...
    if (gameMode === 'multi') {
      ...
      return () => {
        console.log('🧹 Multiplayer game cleanup - closing socket and stopping voice');
        socket.close();
    ...
    // Clean up any ongoing speech when leaving the game
    return () => {
      console.log('🧹 Game component unmounting - stopping all voice and sounds');
      voiceService.stopAllAudio(true);
      stopSpeaking();
  ...
  const handleResume = () => {
    if (isPaused && pauseStartTime) {
      ...
      if (pausedSpeechText && speechStartTime && isVoiceEnabled()) {
        ...
        const wordsSpoken = Math.floor(speechDuration / 1000 * wordsPerSecond);
  ...
  useEffect(() => {
    if (gameType === 'time' && gameTimeRemaining > 0 && !gameEnded && !isPaused) {
  ...
  }, [correctAnswers, isQuestionAnswered, currentQuestionIndex, questions, gameMode, gameType, rewardProgress]);
  
  // Game over check
  useEffect(() => {
    if (
      (gameType === 'question' && currentQuestionIndex >= (questions?.length || 10)) ||
      (gameType === 'time' && gameTimeRemaining <= 0)
    ) {
      setGameEnded(true);
      
      // Stop voice narration when game ends (for all game modes)
      console.log(`🎮 Game ended (${gameMode} mode) - stopping voice narration`);
      voiceService.stopAllAudio(false);
      stopSpeaking();
          ...
          try {
            ...
            for (let i = 0; i < playerCount; i++) {
              ...
              if (playerStat && playerName) {
                ...
                if (response.ok) {
                  console.log(`Local multiplayer score saved successfully for ${playerName}`);
                } else {
                  console.error(`Failed to save local multiplayer score for ${playerName}`);
                }
              }
            }
      ...
      if (correctAnswers > 0) {
        ...
        if (gameType === 'question' && correctAnswers === (questions?.length || 10)) {
  ...
  }, [currentQuestionIndex, questions, gameType, gameTimeRemaining, gameMode, correctAnswers, incorrectAnswers, score, stats.averageTime, category, difficulty]);
  ...
  const handleAnswer = (answer: Answer, timeSpent: number) => {
    ...
    if (answer.isCorrect) {
      ...
      if (timeSpent < 5) {
        ...
        playSound('applause');
        playBasicSound('applause'); // Use both sound systems
        setAvatarMessage("Splendid! As Noah built the Ark with faith, you build your knowledge!");
      ...
      playSound('buzzer');
      playBasicSound('buzzer'); // Use both sound systems
      setAvatarMessage("A brave attempt, but fear not, for wisdom grows with each question.");
    ...
    if (gameMode === 'multi') {
      ...
      setPlayerStats((prevStats: any[]) => {
        ...
        updatedStats[currentPlayerIndex] = {
          ...
          correctAnswers: answer.isCorrect ? playerStat.correctAnswers + 1 : playerStat.correctAnswers,
          incorrectAnswers: !answer.isCorrect ? playerStat.incorrectAnswers + 1 : playerStat.incorrectAnswers,
          totalTimeSpent: playerStat.totalTimeSpent + timeSpent,
          averageTime: (playerStat.totalTimeSpent + timeSpent) / 
  ...
  const handleNextQuestion = () => {
    ...
    if (gameMode === 'multi' && playerCount > 1) {
      ...
      setAvatarMessage(`${playerNames[nextPlayerIndex]}'s turn! Let's see how well you know the Bible!`);
    } else {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary via-primary-dark to-secondary-dark flex items-center justify-center">
        <div className="text-center bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20 shadow-xl">
          ...
          <p className="text-white/80">Preparing your Biblical Trivia Quest...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary via-primary-dark to-secondary-dark flex items-center justify-center">
        <div className="text-center bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20 shadow-xl max-w-md mx-4">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Failed to Load Questions</h2>
          <p className="text-white/80 mb-6">There was an error loading the trivia questions. Please try again.</p>
          <button 
            onClick={() => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary via-primary-dark to-secondary-dark flex items-center justify-center">
        <div className="text-center bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20 shadow-xl max-w-md mx-4">
          ...
          <p className="text-white/80 mb-6">There are no questions available for the selected category and difficulty.</p>
          <button 
            onClick={() => {
              // Stop voice narration before navigating
              voiceService.stopAllAudio(true);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary via-primary-dark to-secondary-dark flex items-center justify-center p-4">
        <div className="text-center max-w-2xl mx-auto bg-white/10 backdrop-blur-sm rounded-3xl p-8 border border-white/20 shadow-2xl">
          ...
          <h2 className="text-5xl font-heading font-bold text-white mb-6">GAME OVER!</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button 
              onClick={() => {
                // Stop voice narration before navigating
                console.log('🏠 HOME button clicked - stopping voice narration');
                voiceService.stopAllAudio(true);
                stopSpeaking();
                setLocation('/');
              }}
            ...
            <button 
              onClick={() => {
                // Stop voice narration before navigating
                console.log('🔄 PLAY AGAIN button clicked - stopping voice narration');
                voiceService.stopAllAudio(true);
                stopSpeaking();
  ...
  return (
    <div id="app" className="min-h-screen bg-gradient-to-br from-primary via-primary-dark to-secondary-dark">
          ...
          <div className="mb-4 bg-gradient-to-r from-white/5 to-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/30 shadow-xl">
            <div className="text-center">
              <div className="flex items-center justify-center mb-4">
                ...
                <h3 className="text-accent text-lg sm:text-xl font-bold font-heading">MULTIPLAYER MODE</h3>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold ${
                        isCurrentPlayer ? 'bg-white text-gray-900' : 'bg-white/20 text-white/80'
                      }`}>
                        {index + 1}
                      </div>
                      
                      {/* Player name */}
                      <span className={`text-sm font-medium ${isCurrentPlayer ? 'text-white' : 'text-white/80'}`}>
                        {name}
                      </span>
              ...
              {currentPlayerIndex !== undefined && playerNames[currentPlayerIndex] && (
          ...
          {currentQuestion && (
            <div className="flex-1">
              <GameBoard
                ...
                totalQuestions={gameType === 'question' ? questions?.length || 10 : '∞'}
                ...
                currentPlayerName={gameMode === 'multi' && playerCount > 1 ? playerNames[currentPlayerIndex] : undefined}
              />
            </div>
          )}
      ...
      {showLeaderboard && (
        <LeaderboardModal
          ...
          onPlayAgain={() => {
            // Stop voice narration before navigating
            console.log('🔄 PLAY AGAIN (Leaderboard) - stopping voice narration');
            voiceService.stopAllAudio(true);
            stopSpeaking();
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-8 max-w-md mx-4 text-center shadow-2xl border border-white/20">
            <div className="mb-8">
              ...
              <h2 className="text-4xl font-bold text-white mb-3">Game Paused</h2>
              ...
              {pausedSpeechText && (
                <div className="mt-6 p-4 bg-accent/10 rounded-xl border border-accent/20">
                  <p className="text-sm text-accent">
                    <strong>Voice Narration:</strong> Will resume from where it left off when you continue.
                  </p>
                </div>
              )}
            </div>
