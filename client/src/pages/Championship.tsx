...
export default function Championship() {
  ...
  return <main className="home-page min-h-screen bg-gradient-to-b from-[#121628] via-[#1a1f3a] to-[#0d1020] text-white p-6 font-heading"><div className="max-w-6xl mx-auto">
    ...
    <div className="grid lg:grid-cols-2 gap-6 mt-8">
      <section className="home-glass-card rounded-2xl p-5"><h2 className="text-xl font-bold mb-4">Standings</h2>
        {data.standings.map((t: any, i: number) => <div className="grid grid-cols-[2rem_1fr_repeat(4,3rem)] gap-2 py-3 border-b border-white/10" key={t.id}>
          <b className="text-amber-400">{i + 1}</b><Link href={`/championship-teams/${t.id}`} className="text-amber-300 hover:underline">{t.name}</Link><span>{t.played}P</span><span>{t.wins}W</span><span>{t.losses}L</span><b>{t.points}pts</b></div>)}
      </section>
      <section className="home-glass-card rounded-2xl p-5"><h2 className="text-xl font-bold mb-4">Matches</h2>
