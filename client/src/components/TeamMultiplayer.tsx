...
export const TeamMultiplayer: React.FC<TeamMultiplayerProps> = ({ 
  ...
  const { data: teams = [] } = useQuery<Team[]>({
    ...
    queryFn: () => apiRequest('GET', `/api/teams?gameSessionId=${gameSessionId}`).then(res => res.json()),
  ...
  const respondToInvitationMutation = useMutation({
    mutationFn: async (data: { invitationId: string; status: "accepted" | "declined" }) => {
      const res = await apiRequest('PATCH', `/api/team-invitations/${data.invitationId}`, { 
        status: data.status 
      });
      return res.json();
    },
    onSuccess: (invitation) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/teams'] });
      toast({
        title: invitation.status === "accepted" ? "Invitation Accepted 3" : "Invitation Declined",
  ...
  useEffect(() => {
    ...
    const setOnlineStatus = async () => {
      try {
        await apiRequest('PATCH', `/api/users/${user.id}/online`, { isOnline: true });
      } catch (error) {
  ...
  const handleInviteUser = async (userId: number) => {
    ...
    try {
      ...
      if (socket.readyState !== WebSocket.OPEN) {
        throw new Error(`WebSocket not connected (state: ${socket.readyState})`);
      }
      ...
      console.log('Sending recruitment message:', recruitmentMessage);
      
      // Send recruitment message
      socket.send(JSON.stringify(recruitmentMessage));
      
      console.log('Recruitment message sent successfully');
      
    } catch (error) {
      console.error('Recruitment failed:', error);
      toast({
        title: "Communication Error",
        description: `Failed to send recruitment: ${error instanceof Error ? error.message : 'Unknown error'}`,
        variant: "destructive",
      });
    } finally {
  ...
  const handleRespondToInvitation = (invitation: TeamInvitation, status: "accepted" | "declined") => {
    if (!socket) {
      toast({
        title: "Connection Error",
    ...
    const eventType = status === 'accepted' ? 'accept_team_invitation' : 'decline_team_invitation';
    ...
    toast({
      title: status === 'accepted' ? "Accepting Invitation..." : "Declining Invitation...",
  ...
  const canStartGame = currentTeam && currentTeam.members.length === 3 && teams.length === 2;
  const isTeamCaptain = currentTeam && currentTeam.captainId === user?.id;
  const bothTeamsReady = teams.length === 2 && teams.every(team => team.members.length === 3);
      ...
      <div className="relative z-10 min-h-screen p-8">
        ...
        <div className="text-center space-y-6 mb-12">
          ...
          <p className="text-xl text-white/80 max-w-2xl mx-auto leading-relaxed">
            Assemble your dream team of 3 biblical scholars and engage in the ultimate scripture showdown!
          </p>
              ...
              <Card className="bg-primary/20 border-primary/40 backdrop-blur-lg shadow-2xl">
                <CardHeader className="pb-6">
                  <CardTitle className="text-white flex items-center gap-4 text-2xl font-bold">
                    ...
                    <div>
                      <div className="text-2xl">{currentTeam ? `Team: ${currentTeam.name}` : 'Team Battle Status'}</div>
                      <div className="text-white/70 text-sm font-normal mt-1">Build your biblical dream team</div>
                    </div>
                  </CardTitle>
                </CardHeader>
                  ...
                  {!currentTeam ? (
                    <div className="text-center p-8 bg-accent/20 border border-accent/40 rounded-xl">
                      <div className="space-y-4">
                        ...
                        <div>
                          <h3 className="text-white font-bold text-xl mb-2">Ready to Start Team Battle?</h3>
                          <p className="text-white/70 text-lg">Recruit your first player from the online champions list to begin forming teams.</p>
                          <p className="text-white/60 text-sm mt-2">The first player you recruit will automatically become captain of the opposing team!</p>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-8">
                      <div className="space-y-4">
                        <div className="flex items-center justify-between mb-6">
                          <h3 className="text-white font-bold text-xl">Team Roster</h3>
                          <Badge variant="outline" className="bg-primary/30 text-white border-primary px-4 py-2 text-lg">
                            {currentTeam.members.length}/3 Players
                          </Badge>
                        </div>
                        {currentTeam.members.map((member, index) => (
                          <div key={member.userId} className="flex items-center justify-between p-6 bg-card/50 border border-primary/30 rounded-xl hover:bg-card/70 transition-all duration-200">
                            <div className="flex items-center gap-4">
                              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-white font-bold text-xl ${
                                member.role === 'captain' ? 'bg-gradient-to-r from-accent to-accent-dark shadow-lg shadow-accent/30' : 'bg-gradient-to-r from-primary to-secondary shadow-lg shadow-primary/30'
                              }`}>
                                {index + 1}
                              </div>
                              <div>
                                <div className="flex items-center gap-3">
                                  <span className="text-white font-bold text-lg">{member.username}</span>
                                  {member.role === 'captain' && (
                                    <Crown className="h-6 w-6 text-accent" />
                                  )}
                                </div>
                                <span className="text-white/70 text-sm">{member.role === 'captain' ? 'Team Captain' : 'Team Member'}</span>
                              </div>
                            </div>
                            <Badge variant={member.role === 'captain' ? 'default' : 'secondary'} className={`px-4 py-2 text-sm font-semibold ${
                              member.role === 'captain' ? 'bg-accent/30 text-black border-accent' : 'bg-secondary/30 text-white border-secondary'
                            }`}>
                        ...
                        {/* Empty slots */}
                        {Array.from({length: 3 - currentTeam.members.length}).map((_, index) => (
                          <div key={`empty-${index}`} className="flex items-center p-6 bg-card/20 border-2 border-dashed border-muted/50 rounded-xl">
                            ...
                            <div>
                              <span className="text-muted-foreground font-medium text-lg">Awaiting Player...</span>
                              <div className="text-muted-foreground/70 text-sm">Invite friends to join your team</div>
                            </div>
                          </div>
                        ))}
                      </div>
                      
                      {currentTeam.members.length < 3 && currentTeam.captainId === user?.id && (
                        <div className="text-center p-6 bg-accent/20 border border-accent/40 rounded-xl">
                          <div className="space-y-2">
                            <p className="text-white font-bold text-lg">
                              Need {3 - currentTeam.members.length} More Player{3 - currentTeam.members.length > 1 ? 's' : ''}
                            </p>
                            <p className="text-white/70">Invite friends using the player recruitment panel →</p>
                          </div>
                        </div>
                      )}
                      ...
                      {!bothTeamsReady && currentTeam && (
                        <div className="text-center p-4 bg-yellow-500/20 rounded-xl border border-yellow-500/40">
                          <p className="text-yellow-200 font-semibold">
                            {teams.length < 2 ? "Waiting for opposing team..." : "Each team needs exactly 3 members to start battle..."}
                          </p>
                ...
                <CardHeader className="pb-6">
                  <CardTitle className="text-white flex items-center gap-4 text-2xl font-bold">
                    ...
                    <div>
                      ...
                      <div className="text-white/70 text-sm font-normal mt-1">Assemble your championship team</div>
                    </div>
                  </CardTitle>
                  <div className="flex items-center gap-3 mt-4">
                    <div className="w-3 h-3 bg-correct rounded-full animate-pulse"></div>
                    <span className="text-white font-semibold text-lg">{onlineUsers.length} Warriors Online</span>
                  </div>
                </CardHeader>
                <CardContent className="space-y-8">
                  {/* Online Players List */}
                  <div className="space-y-4">
                    ...
                    <div className="max-h-80 overflow-y-auto space-y-3">
                      {onlineUsers.map((onlineUser) => (
                        <div key={onlineUser.id} className="flex items-center justify-between p-6 bg-card/50 border border-secondary/30 rounded-xl hover:bg-card/70 transition-all duration-200 hover:scale-102">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <span className="text-white font-bold text-lg">{onlineUser.username}</span>
                              <div className="flex items-center gap-2 mt-1">
                                <div className="w-2 h-2 bg-correct rounded-full animate-pulse"></div>
                                <span className="text-xs text-white/70 font-medium">Ready for Battle</span>
                              </div>
                            </div>
                          </div>
                          {user && onlineUser.id !== user.id && (
                            <Button
                              ...
                              className="bg-gradient-to-r from-secondary to-accent hover:from-secondary-dark hover:to-accent-dark text-white font-bold px-6 py-3 rounded-xl shadow-lg game-button"
                            >
                              {isSendingInvite ? (
                      ...
                      {onlineUsers.length === 0 && (
                        <div className="text-center text-muted-foreground py-12">
                          ...
                          <p className="font-bold text-xl mb-2">No Champions Online</p>
                          <p className="text-muted-foreground/70">Invite friends via email to join the battle</p>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {invitations.filter(inv => inv.status === 'pending' && inv.inviteeId === user?.id).length > 0 && (
              ...
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-4 text-2xl font-bold">
                  ...
                  <div>
                    ...
                    <div className="text-white/70 text-sm font-normal mt-1">You've been recruited for battle!</div>
                  </div>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                {invitations
                  .filter(inv => inv.status === 'pending' && inv.inviteeId === user?.id)
                  .map((invitation) => (
                    <div key={invitation.id} className="flex items-center justify-between p-6 bg-card/50 border border-accent/30 rounded-xl">
                      <div className="flex items-center gap-4">
                        <div className="p-3 bg-accent/30 rounded-xl">
                          <Users className="h-6 w-6 text-black" />
                        </div>
                        <div>
                          <span className="text-white font-bold text-lg">Team Battle Invitation</span>
                          <div className="flex items-center gap-2 mt-1">
                            <Clock className="h-4 w-4 text-accent" />
                            <span className="text-white/70 text-sm font-medium">
                              {Math.ceil((new Date(invitation.expiresAt).getTime() - Date.now()) / 60000)} minutes remaining
                            </span>
                          </div>
                        </div>
                      </div>
