...
import { AlertCircle, Eye, EyeOff, LogIn, Mic } from "lucide-react";
import { cn } from "@/lib/utils";

const commentatorLoginSchema = z.object({
  username: z.string().min(1, "Username is required"),
  password: z.string().min(1, "Password is required"),
});

type CommentatorLoginValues = z.infer<typeof commentatorLoginSchema>;
...
export default function CommentatorLoginPage() {
  ...
  useEffect(() => {
    if (!isLoading && user?.isCommentator && !user.isAdmin) {
      setLocation("/commentator");
    }
  }, [user, isLoading, setLocation]);

  const form = useForm<CommentatorLoginValues>({
    resolver: zodResolver(commentatorLoginSchema),
    defaultValues: { username: "", password: "" },
  });

  const onSubmit = (values: CommentatorLoginValues) => {
    setAccessError(null);
    loginMutation.mutate(values, {
      onSuccess: (loggedInUser) => {
        if (loggedInUser.isAdmin || !loggedInUser.isCommentator) {
          setAccessError("This account does not have commentator access.");
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">Commentator Desk</h1>
            <p className="text-white/60 text-sm">
              Sign in with a commentator account to follow the live championship match
          ...
          <div className="home-glass-card rounded-2xl p-6 sm:p-8 border border-white/10">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder="Enter commentator username"
                ...
                {(accessError || loginMutation.isError) && (
                  <div className="flex items-start gap-2 rounded-xl bg-red-500/10 border border-red-500/25 px-3 py-2.5 text-sm text-red-300">
                    ...
                    <span>
                      {accessError || loginMutation.error?.message || "Login failed. Check your credentials."}
                    </span>
                  </div>
                )}
                ...
                >
                  ...
                  {loginMutation.isPending ? "Signing in…" : "Sign in as Commentator"}
                </Button>
              </form>
            </Form>
          </div>
