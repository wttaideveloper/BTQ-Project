...
async function createDatabase() {
  try {
    ...
    // NOTE: This script is primarily for local setup; DATABASE_URL controls the app DB target
    const defaultConnectionString = process.env.DATABASE_URL || "postgresql://postgres:postgres_password123@localhost:5432/postgres";
    const client = postgres(defaultConnectionString);
    
    // Get database name from connection string or use default
    const dbName = (process.env.DATABASE_URL || defaultConnectionString).split('/').pop() || "bible_trivia_db";
